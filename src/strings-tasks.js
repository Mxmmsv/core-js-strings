...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const positive = vowels.includes(str[i]);
    if (positive === true) {
      sum += 1;
    }
  }
...
function isPalindrome(str) {
  ...
  const strOutChr = str
    ...
    .reverse()
    .join('');
  if (str.toLowerCase().replace(/[^a-zA-Z]/g, '') === strOutChr) {
    result = true;
  }
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strToArray.length; i += 1) {
    reverseWord = strToArray[i].split('').reverse().join('');
    finalFinal.push(reverseWord);
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      ...
      !/[!?,@#$ 0-9]/.test(str.charAt(i))
    ) {
      strToUpper += str[i].toUpperCase();
    }
    if (str[i].toUpperCase() === str[i]) {
      strToUpper += str[i].toLowerCase();
    }
  }
  return strToUpper;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((encode) => rot13[alphabet.indexOf(encode)])
    .join('');
}
